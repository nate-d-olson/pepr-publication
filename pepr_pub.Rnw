...
The long DNA fragments used in optical mapping (average size \textgreater 200 Mb) allows for the indentification of large misassemblies (\textgreater 3 kb) not easily identified using short-read sequencing data \cite{Mendelowitz2014}. 
Optical mapping technologies and large insert mate-pair and synthetic long-read library preparation methods \cite{McCoy2014}, are potential alternatives to optical mapping for identifying large misassemblies. 
...
While, contaminants identified by the \texit{Genomic Purity} pipeline are most likely from reagents and due to bioinformatic errors, a conservative estimate of the material purity, assuming all contaminants are real, reported by the pipeline. 
Users will want to take into consideration the limited specificity of the taxonomic classification method. 
