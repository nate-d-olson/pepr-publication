...
A software package, \texttt{peprr}, was developed for the statistical computing language R \citet{R2015} to compile the output from the genome evaluation, characterization, and genomic purity pipelines. 
The compiled data was formatted into a series of data tables within an SQLite, \texttt{peprDB}, database to facilitate downstream analysis \citet{wickham2014tidy}. 
...
Next, sequencing reads are mapped to the reference genome using BWA mem algorithm \citet{Li2013c}. 
Finally, Pilon is used to evaluate and polish the reference assembly \citet{Walker2014}. 
...
IonTorrent PGM data using the TMAP algorithm \citet{Homer}. 
Additionally, if Pacific Biosciences (PacBio) sequencing data is used to generate the input reference assembly. 
The PacBio sequencing data is mapped to the reference genome using the BWA mem algorithm \citet{Li2013c} and  Next the sequence alignment files are processed prior to downstream analysis by marking duplicates with Picard's MarkDuplicates command (\url{http://broadinstitute.github.io/picard}) and realigning reads mapping to regions with insertions or deletion using the GenomeAnalysisToolKit \citet{McKenna2010,DePristo2011}. 
After refining the alignment files, base level analysis is performed using the short-read sequencing data. 
For each platform a VCF (variant call format) file with a number of summary statistics is generated using SAMtools mpileup \citet{Li2009}. 
...
Homogeneity analysis, a measure of genomic content similarity between vials of the reference material, is performed by first generating a pileup file using SAMtools mpileup for each dataset then performing pairwise tumor-normal variant calling using VarScan \citet{Koboldt2009}. 
...
\label{method:4}
The Genomic Purity Pipeline assesses the purity of the genomic material, in terms of the presence of DNA from sources other than the expected genus was assessed using the metagenomic taxonomic read classification algorithm PathoScope 2.0 \citet{Hong2014}. 
...
Using short-read sequencing data as input PathoScope 2.0 first filters and trims low-quality reads (PathoQC), followed by mapping reads to a reference database (PathoMap - a wrapper for bowtie2 \citet{Langmead2012}), then the expectation-maximization algorithm (PathoID) is used for the taxonomic classification. 
...
Pipelines for Evaluating Prokaryotic References (PEPR) uses biological and technical replicate sequencing data from orthogonal sequencing platforms to characterize a microbial material's genome. 
...
In the following section characterization results for the NIST \textit{S. aureus} candidate reference material are presented along with a discussion of the assumptions and limitations of PEPR. 
...
For the \textit{S. aureus} RM, the reference assembly was constructed from Pacific Biosciences (PacBio) RSII long read sequencing data.
%% add reference to supplemental 
To validate the \textit{S. aureus} reference assembly prior to evaluation with PEPR, a whole genome map was generated with the OpGen optical mapping technology. 
The long DNA fragments used in optical mapping (average size \textgreater 200 Mb) allows for the indentification of large misassemblies (\textgreater 3 kb) not easily identified using short-read sequencing data \citet{Mendelowitz2014}. 
Optical mapping technologies and large insert mate-pair and synthetic long-read library preparation methods \citet{McCoy2014}, are potential alternatives to optical mapping for identifying large misassemblies. 
...
Pilon was chosen for the evaluation step as it assesses the accuracy of the genome and corrects errors in the assembly \citet{Walker2014}. 
Other methods are available for evaluating reference genomes, e.g. amosValidate \citet{Phillippy2008} and ALE \citet{Clark2013}, however these methods only assess assembly accuracy without correcting misassemblies. 
...
While PEPR only uses short-read sequencing data to evaluate the material, summary statistics for additional sequence data, such as long read data used to generate the reference assembly, can be calculated.  
...
Based on the sequencing methods used \emph{REFERENCE SUPPLEMENTAL MATERIAL}, longer reads were expected for PGM. The shorter read length is potentially due to the low GC content, which is known to challenge current sequencing technologies \citet{Quail2012}.
...
The positions with low purity for MiSeq were non-uniformly distributed whereas positions with low purity for PGM were uniformly distributed (Fig. \ref{fig:purityPositionFig}). The difference in low-purity position distributions are due to differences in the systematic sequencing error profiles for the two platforms.  PGM has a higher error rate for homopolymers, whereas MiSeq has more contect specfic sequencing error profile \citet{Quail2012}. 
...
Use of additional metrics and algorithms developed for the identification of low-frequency variants, such as loFreq \citet{Wilm2012}, could help identify positions with low levels of biological variability, but are not currently implemented in PEPR. 
...
Material homogeneity was assessed through pairwise statistical analysis of the replicate MiSeq datasets using the VarScan somatic variant caller \citet{Koboldt2009}. 
...
Short-read sequencing data was used to identify the proportion of DNA in the material from an organism other than the material genus, in this case \textit{Staphylococcus}, using PathoScope 2.0 \citet{Hong2014}. 
...
Genomic contaminants can be from the culture itself or reagents and materials used to prepare the material or during sequencing \citet{Shrestha2013,Tang2003,Salter2014}. 
Contaminants identified by the Genomic Purity Pipeline may not be present in the material. 
For example reagents used during library preparation may include contaminants \citet{Tanner1998,Newsome2004,Motley2014,Salter2014}. 
...
\textit{E. coli} is a well-documented contaminant of molecular biology reagents, and not likely a true contaminant \citet{Salter2014}. Lower abundant contaminants may be bioinformatic errors and not true contaminants. 
...
PEPR outputs include a corrected reference genome assembly, identified genome position with high and low purity based on biological and technical variation, base level homogeneity of the material, as well as the percentage and identity of genus level genomic contaminants. 
