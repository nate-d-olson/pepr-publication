...
Well characterized, homogeneous, and stable microbial genomic reference materials can be used to evaluate measurement processes, improving confidence in microbal whole genome sequencing results. 
We have developed a reproducibile and transparent bioinformatics tool, "PEPR", Pipelines for Evaluating Prokaryotic References, for characterizing the reference genome of prokaryotic genomic materials. 
PEPR evaluates the quality, purity, and homogeneity of the reference material genome, and purity of the genomic material.
The quality of the genome is evaluated using high coverage paired-end sequence data; coverage, paired-end read size and direction, as well as soft-clipping rates are used to identify missassemblies. 
The homogeneity and purity of the material relative to the reference genome is characterized by comparing base calls from replicate datasets generated using multiple sequencing technologies. 
...
Pipelines for Evaluating Prokaryotic References (PEPR) is designed to use biological and technical replicate sequencing data from orthogonal sequencing platforms to characterize a microbial material's genome. 
There are two primary reasons for using replicate sequencing datasets. 
One is to test for homogeneity within the batch of DNA being characterized. 
...
PEPR consists of three pipelines: genome evaluation, genome characterization, and genomic purity assessment. 
The three pipelines are discussed below results from characterizing the NIST \textit{S. aureus} candidate reference material provides an example application of the pipelines. 


\subsection{Preparation of Reference Assembly} 
Prior to running PEPR a high quality genome is required. 
For the \textit{S. aureus} RM the reference assembly was constructed from Pacific Biosciences RSII long read sequencing data. 
To validate the \textit{S. aureus} reference genome prior to evaluating with PEPR, a whole genome map was generated with the OpGen optical mapping technology. 
The long DNA fragments (average size \textgreater 200 Mb) allow for the evaluation of large misassemblies (\textgreater 3 kb) that are not easily identified using short-read sequencing data \cite{Mendelowitz2014}. 
Optical mapping technologies and large insert mate-pair library preparation methods, including Illumina's TruSeq Synthetic Long-Reads \cite{McCoy2014}, represent orthogonal methods that can also be used to identify large misassemblies. 
In this work, after the initial assembly was validated for large misassemblies, the assembly was characterized using PEPR. 
The PEPR \textit{Genome Evaluation} pipeline provides an automated method for evaluating and refining a reference genome sequence. 

\subsection{Genome Evaluation}
The first step in PEPR is the \textit{Genome Evaluation} pipeline. 
Pilon was chosen for the evaluation step as it assesses the accuracy of the genome and corrects errors in the assembly. 
Other methods are available for evaluating reference genomes, e.g. amosValidate \cite{Phillippy2008} and ALE \cite{Clark2013}, however these methods only assess assembly accuracy without correcting missassemblies. 
\emph{ADD DISCUSSION OF PAGIT AND REAPR POTENTIALLY OTHER ASSEMBLY VALIDATION METHODS}
...
The candidate genome, once evaluated, and if nessessary refined during the \textit{Genome Evaluation} pipeline, is used as input for the \texit{Genome Characterization} step. 


\subsection{Genome Characterization}
The \textit{Genome Characterization} pipeline calculates base level statistics using replicate sequencing data from orthogonal measurement methods. 
As part of the characterization stage, summary statistics are generated for the user provided sequencing datasets. 
While PEPR only uses short-read sequencing data to evaluate the material, summary statistics can be calculated for additional sequence data, such as long read data used to generate the reference assembly 
The results from the \textit{Genome Evaluation} and \textit{Genome Characterization} pipelines are loaded into peprDB, a SQLite database. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Need to work on rephrasing
Based on the sequencing methods used \emph{REFERENCE SUPPLEMENTAL MATERIAL}, longer reads were expected for PGM. The shorter read length is potentially due to the low GC content, which is known to challenge current sequencing technologies \cite{Quail2012}.
%%%%
%%%%
 
The higher throughput and paired-end reads resulted in a higher coverage for MiSeq compared to PGM (\Sexpr{paste0(as.character(mean_miseq_library_coverage),"X")} vs. \Sexpr{ paste0(as.character(mean_pgm_library_coverage),"X")}). 
...
Through comparison of the base purity for two orthogonal sequencing methods we identified genome positions with low purity values due to platform specific systematic sequencing errors. 
...
A number of base level metrics, such as strand bias, are calculated as part of the PEPR Genome Characterization Pipeline and are included in the pipeline results database. These metrics can be used to differentiate positions with low purity due to measurement error and those due to biological variability. 
...
\emph{LET ME KNOW IF YOU THINK THIS IS BETTER IN THE RESULTS AND DISCUSSION OR CONCLUSION SECTION}
...
Additionally, we would like to thank Drs. Nancy Lin and Steven Lund for converations and suggestions while developing PEPR and drafting the manuscript. 
