...
\title{PEPR: Pipeline for Evaluating Prokaryotic References}
...
\begin{enumerate}
    ...
    \item If a good reference assembly does not exist for the sample, then long-read sequencing like PacBio may be required to generate an assembly, and ideally mapping technologies would be used to validate the assembly.  If a high-quality assembly already exists, then this step is optional.
    ...
    \item PEPR can then be run to assess base-level purity, contaminants, homogeneity, and mis-assemblies.
