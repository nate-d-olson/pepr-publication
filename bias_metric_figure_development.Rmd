...
pur_dat_id <- dplyr::tbl(db_con, "pur_join") %>%
                dplyr::filter(CHROM %in% chrom_names) %>%
                dplyr::collect() %>% mutate(plat1_group = ifelse(plat1 < 0.99, "low","high"),
...
```{r}
min_val <- min(c(pur_dat_id_filt$plat1, pur_dat_id_filt$plat2)) #%>% round(digits = 1)
...
cb_pgm_metrics <- dplyr::tbl(db_con, "cb_pgm") %>%
    dplyr::filter(CHROM %in% chrom_names, SAMPLE == pgm_datasets[1]) %>%         
...
ggplot(cb_pgm_metrics) + 
        ...
        theme_bw() + ggtitle("PGM Metrics Bin Values")
...
cb_pgm_metrics_bin <- cb_pgm_metrics %>% 
                        ...
                        ungroup() %>% 
                        mutate(metric = paste("pgm", metric, sep = "_"))

cb_miseq_metrics_bin <- cb_miseq_metrics %>% 
                            ungroup() %>% 
                            mutate(metric = paste("miseq", metric, sep = "_"))
...
ggplot(pur_dat_id_metric_bin) + geom_bar(aes(x = as.fac  tor(miseq_MQSB))) + facet_wrap(~pur_group, scale = "free_y", ncol = 1) + theme_bw() + ggtitle("MQSB MiSeq Bins")
```
